...
async function submitOperation(event) {
  ...
  if (operationType === "move" && (!targetLocation || !findLocation(targetLocation))) return showToast("请选择有效目标库位");
  if (operationType === "move" && findLocation(targetLocation)?.status === "冻结") return showToast("目标库位已冻结");

  setFormSubmitting(event.target, true);
  try {
  const operationPayload = { type: operationType, sku, batch, qty: rawQty, location, targetLocation, status, note, expectedVersion: selectedOperationVersion };
  try {
  ...
  addLog({ type: operationType, sku, batch, qty, location, targetLocation, status, note });
  saveState();
  resetOperationForm(event.target);
  selectedOperationVersion = null;
  selectedOperationStock = null;
...
function renderOperationStockRows(rows) {
  $("#operationStockList").innerHTML = rows.length
    ? rows.map((item) => {
        ...
        return `
          <button class="data-card compact-stock" type="button" data-op-stock="1" data-sku="${escapeHtml(item.sku)}" data-name="${escapeHtml(item.name || material?.name || "")}" data-batch="${escapeHtml(item.batch)}" data-location="${escapeHtml(item.location)}" data-status="${escapeHtml(item.status)}" data-qty="${item.qty}" data-version="${item.version || 1}">
            <div>
...
function selectOperationStock(event) {
  ...
  $("#materialNameInput").value = findMaterial(card.dataset.sku)?.name || "";
...
function renderSelectedStockInfo() {
  ...
  $("#selectedStockInfo").innerHTML = row
    ? `<strong>已选择：${escapeHtml(location)}</strong><span>${escapeHtml(sku)} / ${escapeHtml(row.name || material?.name || "")} / ${escapeHtml(batch)} / ${escapeHtml(status)} / 现有 ${row.qty}</span>`
    : "";
}
...
function syncOperationSelection() {
  ...
  const location = normalize($("#locationInput").value);
  const status = $("#statusInput").value;
  if (!selectedOperationMatches(sku, batch, location, status)) {
    selectedOperationStock = null;
    selectedOperationVersion = null;
  }
}
...
function renderCountStockList(rows = []) {
  $("#countStockList").innerHTML = rows.length
    ? rows.map((item) => {
        const material = findMaterial(item.sku);
        return `
        <button class="data-card compact-stock" type="button" data-sku="${escapeHtml(item.sku)}" data-name="${escapeHtml(item.name || material?.name || "")}" data-batch="${escapeHtml(item.batch)}" data-location="${escapeHtml(item.location)}" data-status="${escapeHtml(item.status)}" data-qty="${item.qty}" data-version="${item.version || 1}">
          <div>
            <strong>${escapeHtml(item.location)}</strong>
            <span>${escapeHtml(item.sku)} / ${escapeHtml(material?.name || "")}</span>
...
function renderSelectedCountInfo() {
  ...
  const location = normalize($("#countLocationInput").value);
  const status = $("#countStatusInput").value;
  const row = selectedCountMatches(sku, batch, location, status) ? selectedCountStock : findStock(sku, batch, location, status);
...
function syncCountSelection() {
  ...
  const location = normalize($("#countLocationInput").value);
  const status = $("#countStatusInput").value;
  if (!selectedCountMatches(sku, batch, location, status)) {
    selectedCountStock = null;
    selectedCountVersion = null;
  }
}
    ...
    ? rows.map((item) => `
        <article class="data-card">
          <div>
            ...
            <span>${state.stock.filter((stock) => stock.location === item.code).length} 条库存</span>
          </div>
          <div class="card-meta">
...
function editMaterial(sku) {
  const material = state.materials.find((item) => item.sku === sku);
  if (!material) return;
...
function editLocation(code) {
  const location = state.locations.find((item) => item.code === code);
  if (!location) return;
...
$("#targetLocationInput").addEventListener("input", () => scheduleLocationOptionSearch($("#targetLocationInput").value));
...
$("#countLocationInput").addEventListener("input", () => scheduleLocationOptionSearch($("#countLocationInput").value));
